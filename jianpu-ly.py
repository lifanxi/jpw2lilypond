...
# v1.144 (c) 2012-2015 Silas S. Brown
...
def score_start(midi):
    ...
    return ret
bar_number_every = 1000000 # TODO customise?  (anyway don't leave it numbering at start of system, doesn't work well in jianpu+lyrics)
...
def score_end(midi,**headers):
    ...
    if headers:
        ...
        ret += r"\header{"+'\n'
        for k,v in headers.items(): ret+=k+'=\markup{\override #\'(font-name . "WenQuanYi MicroHei")"'+v+'"}\n'
        ret += "}\n"
    if midi == 1:
        ret += r"\midi { \context { \Score tempoWholesPerMinute = #(ly:make-moment 84 4)}}" # TODO: make this customisable (and/or check how to print BPMs in jianpu)
    else:
        ret += r"\layout{}"
    return ret + " }"
